...
Use the bundled `scripts/install.sh` to install this skill for another agent runtime. The script accepts a local ZIP or an HTTP(S) archive and supports `codex`, `claude`, `agents`, `grok`, or `all` targets.
...
# One-command distribution after publishing the script and ZIP at public URLs
curl -fsSL https://YOUR_HOST/install.sh | bash -s -- \
  --all --archive https://YOUR_HOST/beginner-english-doodle-cards.skill.zip
```

Do not claim that the remote one-liner is live until the script and ZIP have been uploaded to a public URL. After installation, the user may invoke `$beginner-english-doodle-cards` in the target agent.
